(function() {
	...
	var prepareSource = function() {
		...
		// Python
		py = '<script type="text/python">' + py + '\n<\/script>';
		src = src.replace('</head>', py + '</head>');
  ...
  js_editor.on('change', function (inst, changes) {
	...
	// PYTHON EDITOR
	cm_opt.mode = 'python';
	var py_box = document.querySelector('#py textarea');
	var py_editor = CodeMirror.fromTextArea(py_box, cm_opt);
  
  py_editor.on('change', function (inst, changes) {
    render();
  });
	
	// SETTING CODE EDITORS INITIAL CONTENT
	html_editor.setValue('<p>Your name? <input id=\"myname\">\n<button onclick=\"echo()\">click!</button>');
	css_editor.setValue('body { color: blue; }');
	py_editor.setValue('def echo():\n    alert((\"Hello %s !\" % doc[\"myname\"].value))');
